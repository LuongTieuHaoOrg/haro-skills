...
1. **No-args case:** scan `.haro-crew/meetings/*/meeting.yaml` for `status: in-progress`. If found, show the list in chat + picker: `Resume <id>` each / `New meeting`. If none (or user picks new), ask topic (1 question + free-text) then goal (1 question + free-text).
...
5. **Debate loop (round-robin):**
   ...
   - Chair translates the summary into `language.response`, renders in chat, and pauses: picker `Next round` / `Redirect (free-text instruction)` / `Conclude meeting`. On conclude: write `conclusion`, set `status: completed`.
