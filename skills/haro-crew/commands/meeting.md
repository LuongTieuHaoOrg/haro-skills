...
1. **No-args case:** scan `.haro-crew/meetings/*/meeting.yaml` for `status: in-progress`. If found — two turns (Turn discipline, `shared/qa-rules.md` §2b): (a) chat-only message listing the in-progress meetings (id | topic | current round); (b) picker with `Resume <id>` options plus `New meeting`. If none (or user picks new), ask topic (1 question + free-text) then goal (1 question + free-text), each as its own chat-then-picker pair.
...
5. **Debate loop (round-robin):**
   ...
   - Chair translates the summary into `language.response` and sends it as a **chat-only message** (no tool call). Only after it is sent, open the picker: `Next round` / `Redirect` (free-text instruction) / `Conclude meeting`. On conclude: write `conclusion`, set `status: completed`.
