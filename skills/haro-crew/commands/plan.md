...
2. **Artifacts:** each meeting lives at `.haro-crew/meetings/MT-YYYYMMDD-HHmmss-<slug>/` from `templates/meeting.yaml` (`status: in-progress`) + `rounds/` subdir. Resuming an `in-progress` meeting: per `shared/question-rules.md` — `<render text>` with the list (id | topic | current round) → `<write history file>` → picker `Resume <id>` / `New meeting`. Chair saves each participant's analysis verbatim to `rounds/round-<N>-<agent_id>.md`, records a 1–2 line English `recap` + `raw_file` pointer plus the structured `summary_of_round` (`consensus | conflicts_or_disputes | key_takeaways | open_questions_for_next_round`) in `meeting.yaml`. Inter-round context is prior summaries + recaps only — never full raw history.
3. **Debate loop (round-robin, capped):** max 2 rounds per topic. Each round ends per `shared/question-rules.md`: `<render text>` with the translated summary in `language.response` → `<write history file>` → `<render popup>` picker `Next round` / `Redirect` (free-text) / `Conclude`. On conclude: write `conclusion`, set `status: completed`.
...
2. **Sign-off pack (per `shared/question-rules.md`, exactly ONCE):**
   - **`<render text>`** (prescribed content only, anti-narration):
   ...
   - **`<write history file>`:** append the full pack Q&A to `.haro-crew/temp/history.md`.
