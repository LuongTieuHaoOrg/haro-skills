...
2. **With-arg case:** if the arg reads as a product idea (no plan exists yet) → treat as the idea, record into `config/project.yaml`, enter Step 1. If it reads as a debate topic (plan exists) → confirm interpretation SHORT (`Discuss '<topic>'? Confirm / Edit`), then enter Step 2 with that topic.
...
2. **Sign-off pack (LONG content — 3-step flow per `shared/question-rules.md`, exactly ONCE):**
