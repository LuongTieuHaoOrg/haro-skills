...
4. **Existing workspace (read-only status):** read `config/project.yaml` → `config/staffing.yaml` → `decisions.yaml` → `tasks.yaml` (read-only). Then per Response Loop (`shared/question-rules.md` §1–§3): `<respond>` with the status (product name + idea 1 line, project type, current phase, decisions confirmed vs `[UNCONFIRMED]`, tasks by status, crew statuses), then write `temp/q-<NN>-<slug>.md` FIRST, then single picker (Mode: single) with the valid next actions derived from state:
