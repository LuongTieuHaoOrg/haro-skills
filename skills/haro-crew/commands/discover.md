...
2. **Per axis (LONG content — 3-step flow per `shared/question-rules.md`):** `<render text>` with the axis context + the 2–4 proposals (prescribed content only, anti-narration) → `<write file>` to `.haro-crew/output/discover-<axis>.md` → `<render popup>` picker/question (Mode: single, or multiple for scope features) whose question points to the file path. Record each answer immediately into `decisions.yaml` (`axis | question | answer | status: confirmed|unconfirmed | decided_at`).
