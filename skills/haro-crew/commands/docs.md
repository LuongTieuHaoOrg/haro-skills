...
4. **Present once (per Response Loop, `shared/question-rules.md` §1–§3):** `<respond>` with the 8 atomic files + one-line brief each, then write `temp/q-<NN>-<slug>.md` FIRST, then single picker `Accept all` / `Edit` (free-text, names the file) / `Regen a view`. One popup for all files, never per-file popups.
5. **Write** accepted files; regen all `_views/` from the atomic sources; seed `.haro-crew/tasks.yaml` from the feature breakdown (each task: id, title, description + acceptance criteria, owner `agent_dev`, `status: pending`). Then list any `temp/q-*.md` files in chat so the user can delete them if wanted.
