...
3. **Synthesize sign-off pack (chat first):**
   ...
   - `[UNCONFIRMED] items:` every default the user never confirmed, each with `Confirm` / `Change` path.
   Then picker (Mode: single per item or batch): `Approve all` / `Change item (free-text)` / `Re-meet on item`.
