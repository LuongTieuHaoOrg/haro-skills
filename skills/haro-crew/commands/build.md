...
5. **Report per task (LONG content — 3-step flow per `shared/question-rules.md`):** `<render text>` in `language.response` (what was built, test evidence, QA verdict, files changed; prescribed content only, anti-narration) → `<write file>` to `.haro-crew/output/build-report-<task-id>.md` → `<render popup>` picker `Next task (recommended)` / `Fix notes now` / `Pause build` / `Stop`, whose question points to the file path. Never point "above".
