...
2. **Pick next task (per Response Loop, `shared/question-rules.md` §1–§3):** `<respond>` with the queue (id | title | status), then write `temp/q-<NN>-<slug>.md` FIRST, then single picker (Mode: single): next `pending` task (recommended) / specific task / `stop`. **AC gate:** a task without recorded acceptance criteria is skipped — route it back for requirements instead of implementing.
3. **Implement:** `<respond>` in chat what will be built (task id + title + AC pointer), then set task `doing`, update `staffing.yaml` (`agent_dev: busy`). Dispatch `agent_dev` with the task brief (description + acceptance criteria + relevant docs pointers). Collect `IMPLEMENTED / TESTS / NOTES`.
4. **Review:** `<respond>` in chat that implementation is done and review is starting (files changed + test evidence pointers), then set task `review`, update staffing (`agent_qa: busy`, `agent_dev: ready`). Dispatch `agent_qa` with task + diff + test evidence. Collect `VERDICT / COVERAGE / GAPS / FOLLOW-UPS`; write `review_note` into `tasks.yaml`.
...
5. **Report per task (per Response Loop, `shared/question-rules.md` §1–§3):** `<respond>` in `language.response` (what was built, test evidence, QA verdict, files changed; prescribed content only, anti-narration), then write `temp/q-<NN>-<slug>.md` FIRST, then `<render popup>` picker `Next task (recommended)` / `Fix notes now` / `Pause build` / `Stop`.
...
7. Set `phase: build` at start. When all tasks are `done` → write the handover (no separate command):
   ...
   - Present per Response Loop (`shared/question-rules.md` §1–§3, pack already persisted in `06-handover.md`): `<respond>` summary in `language.response`, then write `temp/q-<NN>-<slug>.md` FIRST, then `<render popup>` picker `Accept handover` / `Request fix` (free-text) / `Continue build`.
   - On accept: set `phase: done`, set all crew `ready` in `staffing.yaml`, then list the `temp/q-*.md` files created during build in chat so the user can delete them if wanted.
