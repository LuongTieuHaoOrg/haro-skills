...
4. **Propose outline** — list sections for the target file, annotating source (which block/folder and which knowledge files). Mark sections depending on `unsure`/`skipped` answers as `[CHƯA XÁC NHẬN]` and list open gaps. Wait for outline approval.
...
8. **Confirm & mark status** — ask `Mark this file as? (UPDATING / RELEASED)`. Choose `RELEASED` only when no blocking gaps remain; otherwise keep `UPDATING` with open gaps carried in the elicitation file. Write the new status into `.haro-docs/config/status.yaml`. `RELEASED` files become ground for later `generate` runs.
