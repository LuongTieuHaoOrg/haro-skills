...
1b. **Agent assessment first — two turns, chat first (Turn discipline, `SKILL.md` Presentation rule item 6):** (a) send one chat-only message (no tool call) with 3–5 lines (`Situation → Assessment → My recommended angle`); (b) only after it is sent, propose files or elicitation questions. When the target depends on unfinished prerequisites, say plainly why returning to the prerequisite is better instead of asking flatly.
...
3. **Guided elicitation with skip/defer (only after step 1b's assessment message was sent — never open questions on an empty chat):**
...
4. **Propose outline with alternatives — two turns, chat first (Turn discipline, `SKILL.md` Presentation rule item 6):** (a) send one chat-only message (no tool call) with the full outline: `Recommended option A (section structure + why it fits)` → `Alternative option B (different structure + one-line tradeoff each side)` → open gaps, marking sections depending on `unsure`/`skipped` answers as `[CHƯA XÁC NHẬN]`; (b) only after it is sent, open the picker (Mode: single) with short labels only — `Choose A (recommended)` / `Choose B (alternative)` / `Revise outline` — never repeat the outline inside the picker payload.
