...
1b. **Agent assessment first — one response, chat text then tool call last (Turn discipline, `SKILL.md` Presentation rule item 6):** block 1 = chat text with 3–5 lines (`Situation → Assessment → My recommended angle`); block 2 (FINAL, same response) = the file proposals or elicitation questions picker. When the target depends on unfinished prerequisites, say plainly why returning to the prerequisite is better instead of asking flatly.
...
3. **Guided elicitation with skip/defer (step 1b's assessment is the chat text block of this same asking flow — never open questions with no assessment text before the tool call):**
...
4. **Propose outline with alternatives — one response, chat text then tool call last (Turn discipline, `SKILL.md` Presentation rule item 6; LONG content):** block 1 = chat text with the full outline: `Recommended option A (section structure + why it fits)` → `Alternative option B (different structure + one-line tradeoff each side)` → open gaps, marking sections depending on `unsure`/`skipped` answers as `[CHƯA XÁC NHẬN]`; block 2 (FINAL, same response) = the picker (Mode: single) with short labels only — `Choose A (recommended)` / `Choose B (alternative)` / `Revise outline` — plus a one-line pointer. Never compress the outline into the picker payload, and never end block 1 without block 2.
...
9. **Next-step popup — one response, chat text then tool call last (Turn discipline, `SKILL.md` Presentation rule item 6):** block 1 = chat text recapping what was just finalized; block 2 (FINAL, same response) = the next-action picker (Mode: single; picker tool when available, otherwise numbered list) with 2–3 concrete candidates (prerequisites + gaps, one-line reason each), plus `review <just-finished file>` and `stop`. Example candidates: `1. generate 02-business/01-value-prop.md — business value needed before architecture; 2. review 01-overview/01-problem-statement.md — just RELEASED, worth a critic pass; 3. stop`. Wait for the pick; do NOT auto-run.
