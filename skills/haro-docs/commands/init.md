...
3. **Synthesize — two turns, chat first (Turn discipline, `SKILL.md` Presentation rule item 6):** (a) send one chat-only message (no tool call) with a **Project Note** (5–8 lines: initialized?, doc-root, docs coverage % RELEASED, top-3 gaps, tech stack, knowledge depth) plus an **Agent take** (2–3 lines: biggest risk, most worrying gap, proposed move + one-line reason); (b) only after it is sent, open the next-command picker.
4. **Branch:**
   - Not initialized → continue the fresh workflow (§5.2 below).
    - Initialized → **refresh mode**: add missing doc files to `config/status.yaml` as `UPDATING` (the only write; never removes or rewrites user content), report the delta in a chat-only message (new files, coverage change, still-open gaps; no tool call), then suggest next commands (`generate`, `review`, `knowledge --ingest`) via picker (Mode: single) and continue with the picked one after reading its workflow file.
...
1b. **Agent analysis — two turns, chat first (Turn discipline, `SKILL.md` Presentation rule item 6):** (a) send one chat-only message (no tool call) with `Situation (3–5 lines from scan) → Assessment (risks, which docs to prioritize, anything suspicious in the stack) → My proposal (which deliverables first + why)`; (b) only after it is sent, proceed to clarifying questions. Drop questions the analysis already answered.
...
When init completes, always (a) send one chat-only message (no tool call) summarizing what was created, then (b) show a next-step picker with **2–3 concrete smart suggestions** derived from the new state, e.g.:
