...
2. **Ensure agents, then resolve reviewers:** when `.haro-docs/config/agents.yaml` is missing or holds zero enabled entries, run the shared setup flow in `shared/agents-setup.md` first (skip = stop the review: report `No reviewers configured` and end). Otherwise resolve reviewers from the file:
   - SHORT single multi-select picker call per `shared/question-rules.md` (Mode: multiple; picker tool when available, otherwise numbered list) with the enabled-agent list (`id | invoke | role`) embedded in the question payload. Pre-select `default_reviewer`. Default mode is **single critic**; multi-agent runs only when the user selects 2+ agents or the topic explicitly needs research + critique.
...
3. **Dispatch reviewers** — each reviewer receives: the topic/file content, the loaded context summary, and its persona: the project copy (`.haro-docs/agents/<file>`, located via `.haro-docs/agents/index.yaml`) when present, else the entry `role + prompt` from `config/agents.yaml`. Dispatch by `invoke`: `task` → Task tool with `subagent_type: <type>`; `command` → ask the runtime to run `<command>` with the same payload (unsupported runtime → inline fallback + note in synthesis); `inline` → run in context. Require a structured return:
...
6. **Next-step popup (LONG content — 3-step flow per `shared/question-rules.md`):** `<render text>` with the synthesis (prescribed content only, anti-narration) → persisted already when step 5 saved (`reviews/RR-*.md`); when skipped, `<write file>` to `.haro-docs/output/review-synthesis-<slug>.md` → `<render popup>` follow-ups picker (Mode: single; picker when available) whose question points to the report file: e.g. `generate <related file>`, `knowledge --ingest <new fact surfaced>`, `review again with more evidence`, `stop`. Wait for the pick; do NOT auto-run.
