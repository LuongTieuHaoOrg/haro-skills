...
6. **Next-step popup — two turns, chat first (Turn discipline, `SKILL.md` Presentation rule item 6):** (a) send the synthesis as a chat-only message (no tool call); (b) only after it is sent, propose follow-ups (Mode: single; picker when available): e.g. `generate <related file>`, `knowledge --ingest <new fact surfaced>`, `review again with more evidence`, `stop`. Wait for the pick; do NOT auto-run.
