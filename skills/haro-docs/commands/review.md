...
2. **Ensure agents, then resolve reviewers:** when `.haro-docs/config/agents.yaml` is missing or holds zero enabled entries, run the shared setup flow in `shared/agent-setup.md` first (skip = stop the review: report `No reviewers configured` and end). Otherwise resolve reviewers from the file:
