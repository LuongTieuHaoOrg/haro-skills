...
2. **Ensure agents, then resolve reviewers:** when `.haro-docs/config/agents.yaml` is missing or holds zero enabled entries, run the shared setup flow in `shared/agents-setup.md` first (skip = stop the review: report `No reviewers configured` and end). Otherwise resolve reviewers from the file:
   - Show enabled agents (`id | invoke | role`) as the chat text block, then close the SAME response with the multi-select picker (Mode: multiple; picker tool when available, otherwise numbered list). Pre-select `default_reviewer`. Default mode is **single critic**; multi-agent runs only when the user selects 2+ agents or the topic explicitly needs research + critique.
...
6. **Next-step popup — one response, chat text then tool call last (Turn discipline, `SKILL.md` Presentation rule item 6):** block 1 = chat text with the synthesis; block 2 (FINAL, same response) = the follow-ups picker (Mode: single; picker when available): e.g. `generate <related file>`, `knowledge --ingest <new fact surfaced>`, `review again with more evidence`, `stop`. Wait for the pick; do NOT auto-run.
