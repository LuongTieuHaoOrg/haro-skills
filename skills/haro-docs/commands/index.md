...
4. **Action picker (SHORT content per `shared/question-rules.md`):** single picker call (Mode: single; picker tool when available, otherwise numbered list) with intro + light status + all 6 commands (`init`, `generate`, `review`, `knowledge`, `config`, `stop`) and one-line "when to use" each embedded in the question payload. No separate text block. When not initialized, pre-suggest `init`. Once picked, follow the MANDATORY ROUTING in `SKILL.md`: read that command's workflow file fully before acting. Do NOT auto-run side effects.
