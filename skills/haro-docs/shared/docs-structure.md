...
`init` creates all folders/files above; each `10-deliverables/*.md` is a placeholder with its own headings plus `Ref: ../01-overview/...` links — not identical templates. In `00-common`, `01-conventions.md` is a read-only view rendered by `init` from `config/project.yaml` (`conventions:`, `audience.technical_depth`, `deliverables`) plus a pointer to the fixed rules (`shared/writing-rules.md`), while `02-references, 03-abbreviations, 04-glossary, 05-traceability` are living references with status `UPDATING`, written only by `generate`; `05-traceability.md` format is two tables: (1) `deliverable | source blocks | block status (from status.yaml) | knowledge refs (from index.yaml)`; (2) ID-level chains `need (N-xx.y) | use case (UC-xx) | story (US-xxx) | AC (AC-xxx.y) | test (TC-xxx) | status`. Every requirement ID from `writing-rules.md` (§12) appears in table (2) — an ID missing from the trace is an open gap. `01-overview` is the starting point for writing (not `00-common`).
